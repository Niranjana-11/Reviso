...
export default function QuestionCard({ item, index, checked, onToggle }) {
  ...
  return (
    <div
      onClick={onToggle}
      style={{
        border: `1.5px solid ${checked ? "#2563eb" : "var(--border-card)"}`,
        borderRadius: 12,
        padding: "14px 16px",
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{
            ...
            {/* Note source — which notes file this came from */}
            {item.note_source && (
              <Chip bg="#fef3c7" color="#92400e">📋 {item.note_source}</Chip>
            )}
            {item.difficulty && (
              <Chip bg={diff.bg} color={diff.color}>
                {item.difficulty}
              </Chip>
            )}
            {item.marks && (
              <Chip bg="#f1f5f9" color="#334155">
                {item.marks} marks
              </Chip>
            )}
            {item.topic && (
              <Chip bg="#f5f3ff" color="#6d28d9">
                📌 {item.topic}
              </Chip>
            )}
          </div>
